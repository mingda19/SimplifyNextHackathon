...
export default function Orders() {
  ...
  // FIX: Added curly braces so this function returns void, not a Promise.
  const load = () => {
    api
      .orders()
      .then((o) => {
  ...
  // FIX: Wrapped in an anonymous function so useEffect doesn't receive a Promise.
  useEffect(() => {
    load();
  }, []);
  ...
  const receiveAll = async () => {
    ...
    if (
      !confirm(
        `Mark all ${open.length} open orders as arrived? This books them into stock.`,
      )
    )
      return;
    ...
    for (const o of open) {
      try {
        await api.receiveOrder(o.order_id);
        ok++;
      } catch {
        failed++;
      }
  ...
  if (err)
    return (
      <>
        ...
        <ServiceDown name="Inventory service" error={err} />
      </>
    );
  if (!orders)
    return (
  ...
  const overdue = open.filter(
    (o) => o.expected_at && new Date(o.expected_at) < new Date(),
  );
  ...
  return (
    <>
      <div className="pagehead">
        <div>
          ...
          <p className="muted small" style={{ margin: 0 }}>
            Stock that is paid for but not yet on the shelf. While an order is
            open its SKU is not re-flagged as low, so the agent will not
            re-order it.
          </p>
        </div>
        <div className="row">
      ...
      <div className="grid g4" style={{ marginBottom: 16 }}>
        ...
        <Stat
          n={overdue.length}
          l="past expected date"
          kind={overdue.length ? "warn" : undefined}
        />
        <Stat
          n={orders.filter((o) => o.status === "FULFILLED").length}
          l="received"
        />
      </div>
        ...
        <div className="card" style={{ overflowX: "auto" }}>
          <table>
            ...
            <tbody>
              {shown.map((o) => {
                const late =
                  o.expected_at && new Date(o.expected_at) < new Date();
                return (
                  <tr key={o.order_id}>
                    ...
                    <td>
                      {o.status === "PLACED" ? (
                        <Pill kind="warn">on the way</Pill>
                      ) : o.status === "FULFILLED" ? (
                        <Pill kind="ok">received</Pill>
                      ) : (
                        <Pill>cancelled</Pill>
                      )}
                    </td>
...
function ReceiveModal({ order, onClose, onDone }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      onDone(
        `Received ${r.qty_received} x ${r.sku} into lot ${r.lot_id}. On hand: ${r.on_hand}.`,
      );
    } catch (ex) {
      setErr(ex.message);
      setBusy(false);
  ...
  return (
    <Modal title={`Receive ${order.sku}`} onClose={onClose}>
      <p className="muted small" style={{ marginTop: 0 }}>
        {order.qty} ordered from {order.vendor_id} - S$
        {(order.total_sgd || 0).toFixed(2)}
      </p>
      <form onSubmit={submit}>
        ...
        <div className="field">
          ...
          <p className="muted small" style={{ marginTop: 6 }}>
            Short deliveries happen. Enter what physically arrived, not what was
            ordered.
          </p>
        </div>
        <div className="field">
          <label>
            Expiry date <span className="muted small">optional</span>
          </label>
          <input
            type="date"
            value={expiry}
          ...
          />
          <p className="muted small" style={{ marginTop: 6 }}>
            Received stock becomes a lot with its own expiry, which is what
            drives FEFO issuing and the expiring-soon alerts. Blank defaults to
            180 days.
          </p>
        </div>
